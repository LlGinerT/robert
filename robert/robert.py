...
def main(exe_type="command", sys_args=None):
    ...
    if args.evaluate:
        print(
            "x  Currently, the EVALUATE module does not work in ROBERT v2.0.1. This module will be available in future versions. Install compatible version: pip install robert==1.2.1"
        )
        sys.exit()
    if args.aqme:
        print(
            "x  Currently, the AQME module does not work in ROBERT v2.0.1. This module will be available in future versions. Install compatible version: pip install robert==1.2.1"
        )
        sys.exit()
    ...
    if not args.evaluate:
        ...
        if full_workflow:
            ...
            args.discard = (
                []
            )  # avoids an error since the variable(s) are removed in CURATE
